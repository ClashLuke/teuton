...
def expected_compute_units(manifest: JobManifestV3, graph: Graph) -> float:
    ...
    return expected_compute_units_from_graph(graph)


def expected_compute_units_from_graph(graph: Graph) -> float:
    """Graph-only CU estimate for orchestrator scheduling before a manifest exists."""
    return max(MIN_CU, estimate_graph_ops(graph) / OPS_PER_CU)
