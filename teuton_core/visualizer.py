"""Read-only bucket snapshots for the Teuton visualizer."""
    ...
    *,
    ...
    for manifest, role in manifests:
        ...
        job = _job_row(
            ...
            audits=job_audits,
            now=now,
            score_points=score_points.get(manifest.job_id, 0.0),
        )
        jobs.append(job)
...
def _job_score_points(
    receipts: dict[str, JobReceiptV3],
    verdicts: dict[str, list[VerificationVerdictV3]],
) -> dict[str, float]:
    """Per-job contribution matching validator score-window accounting."""
    by_hotkey: dict[str, dict[str, float]] = {}
    job_cu: dict[str, float] = {}
    job_bucket: dict[str, str] = {}

    for job_id, receipt in receipts.items():
        hotkey = receipt.worker.hotkey_ss58
        totals = by_hotkey.setdefault(hotkey, {"pass": 0.0, "fail": 0.0, "unsampled": 0.0})
        verdict = _primary_verdict(verdicts.get(job_id, []))
        replay_compute_sec = verdict.replay_compute_sec if verdict is not None else 0.0
        cu = _estimated_cu(receipt, replay_compute_sec)
        job_cu[job_id] = cu
        if verdict is None:
            totals["unsampled"] += cu
            job_bucket[job_id] = "unsampled"
        elif verdict.status == "pass":
            totals["pass"] += cu
            job_bucket[job_id] = "pass"
        elif verdict.status == "fail":
            totals["fail"] += cu
            job_bucket[job_id] = "fail"
        else:
            totals["unsampled"] += cu * 0.5
            job_cu[job_id] = cu * 0.5
            job_bucket[job_id] = "unsampled"

    trust_by_hotkey: dict[str, float] = {}
    for hotkey, totals in by_hotkey.items():
        checked = totals["pass"] + totals["fail"]
        if checked == 0:
            trust = 0.25
        elif totals["fail"] > 0:
            trust = max(0.0, (totals["pass"] - 2.0 * totals["fail"]) / checked)
        else:
            trust = 1.0
        trust_by_hotkey[hotkey] = trust

    out: dict[str, float] = {}
    for job_id, receipt in receipts.items():
        bucket = job_bucket.get(job_id)
        if bucket == "pass":
            out[job_id] = job_cu.get(job_id, 0.0)
        elif bucket == "fail":
            out[job_id] = 0.0
        else:
            out[job_id] = job_cu.get(job_id, 0.0) * trust_by_hotkey.get(receipt.worker.hotkey_ss58, 0.25)
    return out


def _primary_verdict(verdicts: list[VerificationVerdictV3]) -> VerificationVerdictV3 | None:
    if not verdicts:
        return None
    return sorted(verdicts, key=lambda v: (v.checked_unix, v.validator_hotkey), reverse=True)[0]


def _estimated_cu(receipt: JobReceiptV3, replay_compute_sec: float = 0.0) -> float:
